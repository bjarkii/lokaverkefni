<?php
	...
	class Users
	{
		...
		* @returns boolean
		*/
		public function newUser($first_name,$last_name,$user_email,$user_name,$user_pass, $user_phone, $securityQuestion, $securityAnswer)
		{
			...
			$statement->bindParam(6,$user_phone);
			$statement->bindParam(7,$securityQuestion);
			$statement->bindParam(8,$securityAnswer);
		...
		/**
		*	@function name:  returnUserID
		*
		*  Retyrns the USER ID
		*
		* @usage example:  $db_object->getUser(666);
		*
		* @param  varchar(15)  user_name
		*
		* @returns int
		*/
		public function returnUserID($user_name)
		{
			$statement = $this->connection->prepare('call ReturnUserID(?)');
		...
		*  Gets all posts
		*
		* @usage example:  $db_object->getSinglePost(666);
		*
		* @returns array
		*/
		public function getAllPosts()
		{
			$statement = $this->connection->prepare('call GetAllPosts()');
		...
		/**
		*	@function name:  getSinglePost
		*
		*  Gets single post via ID
		*
		* @usage example:  $db_object->getSinglePost(666);
		*
		* @param  int  post_id
		...
		*/
		...
		{
			$statement = $this->connection->prepare('call GetSinglePost(?)');
		...
		/**
		*	@function name:  getPostCount
		*
		*  How many posts
		*
		* @usage example:  $db_object->getPostCount();
		*
		* @returns array
		*/
		public function getPostCount()
		{
			$statement = $this->connection->prepare('call GetPostCount()');
		...
		*	@returns boolean
		*/
		public function updateUser($userI_id,$first_name,$last_name,$user_email,$user_name,$user_pass, $user_phone)
		{
			...
			$statement->bindParam(7,$user_phone);
		...
		*	Add new Images.
		*
		*	@usage example:	$db_object->newUser(iamgename, imagepath);
		*
		*	@param var image_name, var image_path
		*
		*	@returns boolean
		*/
		public function newImage($imageName, $iamgeTitle, $imageDesc)
		{
			$statement = $this->connection->prepare('call NewImage(?,?,?)');
			$statement->bindParam(1,$imageName);
			$statement->bindParam(2,$iamgeTitle);
			$statement->bindParam(3,$imageDesc);
		...
		*	@returns boolean
		*/
		public function getImgID($imageName)
		{
			$statement = $this->connection->prepare('call GetImgID(?)');
			$statement->bindParam(1,$imageName);
